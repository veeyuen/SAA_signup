...
def active_invoiced_entry_ids(rows: list[dict[str, Any]]) -> set[str]:
    return {
        _clean(row.get("ENTRY_ID"))
        for row in active_invoice_lines(rows)
        if _clean(row.get("ENTRY_ID"))
    }


def consolidated_invoice_entries(
    *,
    entries: list[dict[str, Any]],
    eligible_order_ids: set[str],
    invoice_rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Return active, not-yet-invoiced event-entry rows across school orders.

    A consolidated MOE invoice is scoped to one school + one competition. The
    caller supplies exactly the order ids in that scope. Entries already present
    on an ISSUED, DISPUTED, or PAID invoice are excluded so invoice snapshots are
    immutable and an entry cannot be billed twice.
    """
    already_invoiced = active_invoiced_entry_ids(invoice_rows)
    out: list[dict[str, Any]] = []
    for entry in entries:
        order_id = _clean(entry.get("ORDER_ID"))
        entry_id = _clean(entry.get("ENTRY_ID"))
        if order_id not in eligible_order_ids or not entry_id:
            continue
        if entry_id in already_invoiced:
            continue
...
def build_consolidated_invoice_line_rows(
    *,
    invoice_id: str,
    invoice_number: str,
    ...
    """Build one immutable MOE_INVOICES row per athlete-event entry line."""
        ...
        rows.append(
            {
                ...
                "ORDER_ID": order_id,
                "COMPETITION_ID": competition_id or _clean(order.get("COMPETITION_ID")),
                "ORGANIZATION_ID": organization_id or _clean(order.get("ORGANIZATION_ID")),
...
def invoice_amount_by_order(lines: list[dict[str, Any]]) -> dict[str, Decimal]:
    totals: dict[str, Decimal] = defaultdict(lambda: Decimal("0"))
    for line in lines:
        order_id = _clean(line.get("ORDER_ID"))
        if order_id:
            totals[order_id] += _d(line.get("LINE_AMOUNT"))
    return dict(totals)
...
def invoice_summary(lines: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "ORDER_ID": order_ids[0] if len(order_ids) == 1 else "",
        "ORDER_IDS": order_ids,
...
def invoice_xlsx_bytes(
    *,
    ...
    rows: list[list[tuple[Any, int]]] = [
        ...
        [("Orders", 2), (", ".join(summary.get("ORDER_IDS", [])), 0)],
    ...
    for line in invoice_lines:
        rows.append(
            [
                ...
                (_clean(line.get("ORDER_ID")), 0),
                (_clean(line.get("ENTRY_ID")), 0),
                (_d(line.get("LINE_AMOUNT")), 3),
            ]
        )
    rows.append([])
    rows.append([("TOTAL", 2)] + [("", 0)] * 7 + [(summary["AMOUNT"], 3)])
...
def invoice_pdf_bytes(
    *,
    ...
    for idx, line in enumerate(invoice_lines, start=1):
        ...
        commands.append(_pdf_text(cols[3][1], y, clipped(line.get("ORDER_ID"), 20), 7))
...
def invoice_email_body(*, summary: dict[str, Any], competition_name: str, paid: bool = False) -> str:
    ...
    order_ids = summary.get("ORDER_IDS", []) or []
    return (
        "Dear School Representative,\n\n"
        f"Singapore Athletics consolidated MOE post-event invoice {summary.get('INVOICE_NUMBER', '')} {status_text}.\n\n"
        f"Competition: {competition_name}\n"
        f"School: {summary.get('ORGANIZATION_NAME', '')}\n"
        f"Orders included: {len(order_ids)}\n"
