...
current_user_email = (
    require_google_login(required_group="admin")
...
# Edit panel
    if st.session_state.edit_idx is not None and 0 <= int(st.session_state.edit_idx) < len(st.session_state.entries):
        ...
        with st.form(key=f"edit_form_{idx}"):
            ...
            _active_division_keys_e = list(_active_divisions().keys())
            div_cur = _coerce_division_key_for_options(
                original.get("event_division", _default_division_key()) or _default_division_key(),
                _active_division_keys_e,
            )
            event_div_e = cL.selectbox(
                "Event Division",
                options=_active_division_keys_e,
                index=_active_division_keys_e.index(div_cur) if div_cur in _active_division_keys_e else 0,
                format_func=_division_display_label,
                key=f"e_event_div_{idx}",
            )
            event_opts_e_raw = allowed_events(gender_to_code(gender_e), event_div_e)
        ...
        if save_btn:
            ...
            if errors:
                ...
                updated.update({
                    ...
                    "event_division": _division_value_for_storage(event_div_e),
                    "event": event_name_e,
