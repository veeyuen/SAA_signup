...
# Roster (Google Sheet) — auto-load for name matching
...
# Output sheet (Google Sheet) — always sync after Add / Edit
...
# Force-enable roster + sync (hidden toggles)
...
# Preload roster once per session (cached by @st.cache_data in loader)
...
if not (st.session_state.get("output_sheet_url") or "").strip():
    st.warning("OUTPUT_SHEET_URL is not set; syncing to Google Sheets will be skipped.")
# ---------------------------------------------------------------------------
...
if not _roster_enabled:
    ...
    st.caption("Roster sheet URL is empty (set ROSTER_SHEET_URL in Streamlit secrets).")
...
if not entries_df.empty:
    ...
    st.caption("Add entries to enable download.")
